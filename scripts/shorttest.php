...
$measurement_json = array(
  "captured_at" => date("Y-m-d\TH:i:s\Z", time()),
  "longitude" => $_GET["lon"],
  "latitude" => $_GET["lat"],
  "value" => $_GET["cpm"],
  "height" => $_GET["alt"],
  "devicetype_id" => $_GET["typ"],
  "device_id" => $_GET["id"],
  "unit" => "cpm" 
);
...
// Log to file
$filename = "logs/" . date('Ymd') . "_" . $measurement_json["device_id"] . "_" .$_SERVER['REMOTE_ADDR'] . ".txt";
...
$filenameCSV = "logs/" . date('Ymd') . "_" .$measurement_json["device_id"] . "_" .$_SERVER['REMOTE_ADDR'] . ".csv";
...
# GeoRSS feed
$filenameRSS = "logs/" . date('Ymd') . "_" .$measurement_json["device_id"] . "_" .$_SERVER['REMOTE_ADDR'] . ".xml";
$xml_entry = '<entry><id>'.uuid("urn:uuid:").'</id><title>'.$measurement_json["value"].' CPM</title><updated>'.$measurement_json["captured_at"].'</updated><georss:point>'.$measurement_json["latitude"].' '.$measurement_json["longitude"].'</georss:point><summary>'.$measurement_json["value"].' CPM</summary></entry>';
$xml_footer = '</feed>';
...
if (!file_exists($filenameRSS)) {
    ...
    $xml_output .= '  <link href="http://api.safecast.org/"/>'.PHP_EOL;
